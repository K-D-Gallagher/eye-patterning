...
dataDir = '/Users/idse/BIG data/Eaton wings/evagination';
...
fileMeta.filenameFormat = '6p_T%u.ome.tif';
fileMeta.timePoints = 0:40;
fileMeta.swapZT = 0;
fileMeta.stackResolution = [0.23 0.23 1]; 
...
customOptions = struct('dilSize', 6, 'erSize', 20, 'areaOpenSize', 1000, 'fillSize', 100);

detectOptions = struct(  'sigma', 1, 'channels', 1, 'zdir', 3,...
...
customOptions = struct('dilSize', 6, 'erSize', 20, 'areaOpenSize', 5000, 'fillSize', 100);

detectOpts = struct(  'sigma', 1, 'channels', 1, 'zdir', 3,...
                        'maxIthresh', 0.1, 'summedIthresh', 0,...
                        'sigZoutliers', 1, 'scaleZoutliers', 50,...
                        'seedDistance', 20, 'customOptions', customOptions); 

xp.detector.setOptions(detectOpts);    
...
inspectOptions= struct('dimension', 'y', 'value', 100, 'pointCloud', 'c');%1150
...
xp.batchProcess(timePoints, batchOptions);
...
for t = 1:nTimePts;
    ...
    frameH = size(M(t).cdata, 1);
    frameW = size(M(t).cdata, 2);
    
    if frameH < minFrameH,  minFrameH = frameH; end
    if frameW < minFrameW,  minFrameW = frameW; end
end

writerObj = VideoWriter(fullfile(dataDir, '3Dshape.avi'));
...
for t = 1:nTimePts
    ...
    M(t).cdata = M(t).cdata(1:minFrameH, 1:minFrameW, :);
