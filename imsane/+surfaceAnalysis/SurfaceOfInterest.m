classdef SurfaceOfInterest < diffgeometry.Manifold2D
    ...
    methods
        ...
        function Aproper = properArea(this, time, mask, chartName)
            ...
            if isempty(this.g(gti).patches)   % added by Andrew Liu (maybe works now???)
                this.NCalcInducedMetric(time);
            end
        ...
        function [metric,stepSize] = getMetricAndrew(this,time,chartName)
            ti = this.tIdx(time);
            if this.dynamic == false,   gti = 1;
            else                        gti = ti; end
        ...
        function lproper = properLengthAndrew(this, time, curve, chartName,metric)
            % calculate proper length of curve
            %
            % lproper = properLength(time, curve, chartName)
            %
